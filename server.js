...
function getCurrentMarket() {
  ...
  if (day >= 1 && day <= 5) {
    ...
    if (hours >= 21 || hours < 4) return 'jsx';
  }
  return null;
}
...
function getMarketStatus() {
  ...
  if (hours >= 9 && hours < 16) {
    return { status: 'NASDAQ/NYSE OPEN', openTime: '9:30 AM', closeTime: '4:00 PM EST' };
  }
  if (hours >= 16 && hours < 21) {
    return { status: 'After Hours', openTime: '9:30 AM', closeTime: '4:00 PM EST', nextMarket: 'JSX (9:00 PM)' };
  }
  if (hours >= 21 || hours < 4) {
    return { status: 'JSX OPEN', openTime: '9:00 PM', closeTime: '4:00 AM EST' };
  }
  
  return { status: 'Pre-Market', openTime: '9:30 AM', closeTime: '4:00 PM EST' };
}
